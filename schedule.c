...
//#define FIB10_ITERATIONS 32
//#define FIB20_ITERATIONS 64
...
void* fib10 (void* thread_id)
{
    ...
    rtclk_goal_time.tv_nsec = rtclk_start_time.tv_nsec;
    rtclk_goal_time.tv_sec = rtclk_start_time.tv_sec + 10;

    while (rtclk_curr_time.tv_sec < rtclk_goal_time.tv_sec && rtclk_curr_time.tv_nsec < rtclk_goal_time.tv_nsec)
    {
        fibonacci(FIB_MAX_SIGNED_INT);
    ...
    printf("fib10 service completed, elapsed: %6.9lf seconds", elapsed);
}
...
void* fib20 (void* thread_id)
{
    ...
    rtclk_goal_time.tv_nsec = rtclk_start_time.tv_nsec;
    rtclk_goal_time.tv_sec = rtclk_start_time.tv_sec + 20;
    ...
    clock_gettime(CLOCK_MONOTONIC, &rtclk_stop_time);
    double elapsed = d_ftime(&rtclk_start_time, &rtclk_stop_time);
    printf("fib20 service completed, elapsed: %6.9lf seconds", elapsed);
}

int main (int argc, char *argv[])
{
    //step 1: initialization
    static struct timespec sleep_time_10 = {10, 0};
    static struct timespec remaining_time_10 = {10, 0};
    static struct timespec sleep_time_20 = {20, 0};
    static struct timespec remaining_time_20 = {20, 0};
    ...
    remaining_time_20.tv_sec = 20;
    sem_post(&fib10_sem);
    nanosleep(&sleep_time_20, &remaining_time_20);
    remaining_time_20.tv_sec = 20;
    sem_post(&fib10_sem);
    nanosleep(&sleep_time_10, &remaining_time_10);
    remaining_time_10.tv_sec = 10;
    sem_post(&fib20_sem);
    nanosleep(&sleep_time_10, &remaining_time_10);
    remaining_time_10.tv_sec = 10;
    sem_post(&fib10_sem);
    ...
    printf("Program completed, elapsed: %6.9lf seconds", elapsed);
